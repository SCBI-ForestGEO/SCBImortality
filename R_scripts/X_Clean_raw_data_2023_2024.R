#############################################################################
# Purpose: clean raw mortality census data and unify all years to standardized format.
# Developed by: Valentine Herrmann - HerrmannV@si.edu. 
# Adapted for 2023 and 2024 Mortality Data by: Rachel Hoffman
# R version 4.0.3 (2020-10-10)

### To be used until 2023 census data has been cleaned by ForestGEO ###
##########################################################################
library(tidyverse)
...
allmort <- read.csv("data/allmort.csv")
...
for(f in paste0("scbi.stem", 1:3)) {
  print(f)
  x <-  read.csv(paste0("/Users/rachelhoffman/Documents/GitHub/SCBI-ForestGEO-Data/tree_main_census/data/census-csv-files/", f, ".csv"))
...
##### adjust scbi.stem4 -- comment out this code if scbi.stem4 has been officially cleaned by ForestGEO #####

scbi.stem4 <- read.csv("/Users/rachelhoffman/Documents/GitHub/SCBI-ForestGEO-Data/tree_main_census/data/census-csv-files/scbi.stem4.csv")
...
unified_colnames <-  read.csv("raw_data/standardizing_colnames.csv")
  ...
  ## For standardizing_columns to function correctly, combine "Status" and "Living Status" columns for 2024 -- may already be in manual fixes
  raw_mortality_2024 <- read.csv("raw_data/Mortality_Survey_2024.csv")  
  raw_mortality_2024 <- raw_mortality_2024 %>%
    mutate(status_2024 = if_else(status_2024 == "LI", living_status_2024, status_2024)) %>%
    mutate(status_2023 = if_else(status_2023 == "LI", living_status_2023, status_2023))
...
### Fix dates for 2023 and 2024
raw_mortality_2023 <- read.csv("raw_data/Mortality_Survey_2023.csv")
raw_mortality_2023 <- raw_mortality_2023 %>%
  mutate(ExactDate = as.Date(cut(as.POSIXct(date_measured, format = "%m/%d/%Y %I:%M:%S %p"), "day")))
raw_mortality_2024 <- raw_mortality_2024 %>%
  mutate(ExactDate = as.Date(cut(as.POSIXct(date_measured, format = "%m/%d/%Y %I:%M:%S %p"), "day")))


## In order for the for loop to function correctly, changes must be saved into csv
write.csv(raw_mortality_2023,"raw_data/Mortality_Survey_2023.csv")
write.csv(raw_mortality_2024,"raw_data/Mortality_Survey_2024.csv")

mort_2023_TEST <- read.csv("/Users/rachelhoffman/Documents/GitHub/SCBImortality/raw_data/Mortality_Survey_2023.csv")
mort_2024_TEST <- read.csv("/Users/rachelhoffman/Documents/GitHub/SCBImortality/raw_data/Mortality_Survey_2024.csv")
...
for(survey_file in survey_files) {
  ...
  if(survey_year > 2020 & survey_year <= 2022) date_format = "%m-%d-%Y"
...
for(survey_year in survey_years) {
  ...
  if(length(setdiff(na.omit(mort$sp), na.omit(scbi.spptable$sp))) > 0) stop ("There is one species that is not in scbi.spptable")
