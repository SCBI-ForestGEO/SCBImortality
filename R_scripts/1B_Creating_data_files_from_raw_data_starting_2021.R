#############################################################################
# Purpose: clean and merge all mortality census data after CI was implemented (in 2021)
...
for(survey_file in survey_files) {
  ...
  # give a record ID
  # mort <- cbind(id = 1:nrow(mort), mort)
  
  
  # fix column names to be consistant
  names(mort) <- tolower(names(mort))
  names(mort) <- gsub("quad", "quadrat", names(mort))
  names(mort) <- gsub("surveyorid", "surveyor", names(mort))
  names(mort) <- gsub("notes.\\d*", "notes", names(mort))
  names(mort) <- gsub("lianas", "liana.load",names(mort)) # *** CHECK SAME PROTOCLOE
  
  names(mort) <- gsub("species", "sp", names(mort))
  # names(mort) <- gsub("^crown.thinning", "fraxinus.crown.thinning", names(mort))
  # 
  ...
  #remove missing tags (not sure whaere they arecming from)
  mort <- mort[!is.na(mort$tag), ]
  
  # deal with duplicated records 
  ## for 2021, 1) select record with surveyor = "BC" or "BC,JJ" (for one quad), 2) select later record (note some are on same submission, so will have same date of survey).
  dup_tags <- paste(mort$tag, mort$stemtag)[duplicated(mort[, c("tag", "stemtag")])]
...
# }))
  ...
  mort$agb.if.dead <-
    round(get_biomass(
      dbh = as.numeric(mort$dbh.if.dead)/10, # in cm
      genus = mort$genus,
