...
#############################################################################
# Purpose: check trees with errors
...
## warning files
e <- read.csv("testthat/reports/requires_field_fix/require_field_fix_error_file.csv")
...
table(e$error_name) # add code if some are not adressed below

# check status history of missing_stems ####
x <- e[e$error_name %in% "missing_stem", ]
...
write.csv(status_history, "temporary_files_for_QA_QC/status_history_of_missing_stems.csv", row.names = F)
