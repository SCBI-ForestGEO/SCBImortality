...
mort <- allmort
# for(sp in "All") { # c("All", sort(unique(allmort$sp)))
#   
...
## get number of dead trees per size class ####
...
## calculate mortality rates per size class ####
mortrate <- 100 * (1 - ((ninit - ndead) / ninit) ^ (1 / mntime))
...
## bootstrap the grids (sample ngrid with replacement) ####
boot_mort <- array(NA, dim = c(dim(dn)[1:2], nreps), 
                   dimnames = c(dimnames(dn)[1:2], list(rep_id = 1:nreps)))
