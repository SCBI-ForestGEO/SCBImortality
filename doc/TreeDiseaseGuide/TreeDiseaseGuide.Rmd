...
mortality_rate_by_species_latest <- mortality_by_species[ which(mortality_by_species$variable=='mortality_rates' & mortality_by_species$survey_year == latest_year),]
names(mortality_rate_by_species_latest)[names(mortality_rate_by_species_latest) == 'sp'] <- 'Tree.species.code'
mortality_rate_by_species_latest$n_latest <- mortality_rate_by_species_latest$N0 - mortality_rate_by_species_latest$n_died
...
tree_species_summary <- read.csv("https://raw.githubusercontent.com/SCBI-ForestGEO/SCBImortality/578811f02ebd863fefb17d8aaa6a18c212fa6930/doc/TreeDiseaseGuide/tree_species_summary.csv") #pulling in an older version of the table with the original species order
## fill in numbers of pests/pathogens from matrix
tree_species_summary$n.FIP<-n_FIP_by_species # this assumes same species order as matrix.csv
tree_species_summary$n.EFIP<-n_EFIP_by_species # this assumes same species order as matrix.csv
tree_species_summary$IUCN.status<-tree_sp_ecology$IUCN_red_list_status # this assumes same species order as SCBI_ForestGEO_sp_ecology.csv
## merge with mortality_rate data frame, then rearrange and delete unwanted columns. THIS REORDERS THE TABLE, WHICH CREATES PROBLEMS
tree_species_summary <- merge(tree_species_summary, mortality_rate_by_species_latest, by = "Tree.species.code", all.x = TRUE)  # merge tables
tree_species_summary$n.living <- tree_species_summary$n_latest # put n_latest in desired column
tree_species_summary = subset(tree_species_summary, select = -c(9:17) )  # remove columns from mortality_rate_by_species_latest
## fill in overall concern level based on other fields
tree_species_summary$overall[] <- NA # clear previous entries
#1. If IUCN Red List status was anything other than "Least Concern", the species was flagged as high concern
tree_species_summary$overall[which(tree_species_summary$IUCN.status != "Least concern") ] <- "H"
...
tree_species_summary$overall[which(tree_species_summary$Tree.species.code != "chvi") ] <- "H"
...
tree_species_summary$overall[which((tree_species_summary$IUCN.status == "Least concern" | is.na(tree_species_summary$IUCN.status)==1) & (tree_species_summary$mortality.rates == "H" | tree_species_summary$AU.status == "H") & tree_species_summary$n.living >= 50) ] <- "H"
tree_species_summary$overall[which((tree_species_summary$IUCN.status == "Least concern" | is.na(tree_species_summary$IUCN.status)==1) & (tree_species_summary$mortality.rates == "H" | tree_species_summary$AU.status == "H") & tree_species_summary$n.living < 50) ] <- "M"
tree_species_summary$overall[which((tree_species_summary$IUCN.status == "Least concern" | is.na(tree_species_summary$IUCN.status)==1) & (tree_species_summary$mortality.rates == "M" & tree_species_summary$AU.status != "H") | (tree_species_summary$AU.status == "M" & tree_species_summary$mortality.rates != "H")  & tree_species_summary$n.living >= 50) ] <- "M"
tree_species_summary$overall[which((tree_species_summary$IUCN.status == "Least concern" | is.na(tree_species_summary$IUCN.status)==1) & (tree_species_summary$mortality.rates == "M" & tree_species_summary$AU.status != "H") | (tree_species_summary$AU.status == "M" & tree_species_summary$mortality.rates != "H")  & tree_species_summary$n.living < 50) ] <- "L"
...
tree_species_summary$overall[which((tree_species_summary$IUCN.status == "Least concern" | is.na(tree_species_summary$IUCN.status)==1) & tree_species_summary$mortality.rates == "L" & tree_species_summary$AU.status == "L") ] <- "L"
#5. Manual patches
tree_species_summary$overall[which(tree_species_summary$Tree.species.code == "chvi") ] <- "H"
## save latest version
