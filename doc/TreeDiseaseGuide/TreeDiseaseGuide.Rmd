...
tree_species_summary <- read.csv("https://raw.githubusercontent.com/SCBI-ForestGEO/SCBImortality/578811f02ebd863fefb17d8aaa6a18c212fa6930/doc/TreeDiseaseGuide/tree_species_summary.csv")
## fill in numbers of pests/pathogens from matrix
tree_species_summary$n.FIP<-n_FIP_by_species
tree_species_summary$n.EFIP<-n_EFIP_by_species
tree_species_summary$IUCN.status<-tree_sp_ecology$IUCN_red_list_status
## merge with mortality_rate data frame, then rearrange and delete unwanted columns. THIS REORDERS THE TABLE
...
tree_species_summary$overall[which(tree_species_summary$IUCN.status == "Least concern" & (tree_species_summary$mortality.rates == "H" | tree_species_summary$AU.status == "H") & tree_species_summary$n.living >= 50) ] <- "H"
tree_species_summary$overall[which(tree_species_summary$IUCN.status == "Least concern" & (tree_species_summary$mortality.rates == "H" | tree_species_summary$AU.status == "H") & tree_species_summary$n.living < 50) ] <- "M"
tree_species_summary$overall[which(tree_species_summary$IUCN.status == "Least concern" & (tree_species_summary$mortality.rates == "M" & tree_species_summary$AU.status != "H") | (tree_species_summary$AU.status == "M" & tree_species_summary$mortality.rates != "H")  & tree_species_summary$n.living >= 50) ] <- "M"
tree_species_summary$overall[which(tree_species_summary$IUCN.status == "Least concern" & (tree_species_summary$mortality.rates == "M" & tree_species_summary$AU.status != "H") | (tree_species_summary$AU.status == "M" & tree_species_summary$mortality.rates != "H")  & tree_species_summary$n.living < 50) ] <- "L"
...
tree_species_summary$overall[which(tree_species_summary$IUCN.status == "Least concern" & tree_species_summary$mortality.rates == "L" & tree_species_summary$AU.status == "L") ] <- "L"
## save latest version
